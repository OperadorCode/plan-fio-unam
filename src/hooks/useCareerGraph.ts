...
export const useCareerGraph = (initialCoursesData: Record<string, Course[]>) => {
        ...
        Object.entries(initialCoursesData).forEach(([year, courses]) => {
            newMap[year] = courses.map(c => {
                ...
                if (c.isElectiveSlot && selectedElectives[c.id]) {
                    ...
                    const selectedOption = group?.find(o => o.id === selectedElectives[c.id]);
    ...
    const baseGraph = useMemo(() => {
        ...
        const newNodes: Node[] = [];
        const sortedYears = Object.keys(columns).map(Number).sort((a, b) => a - b);
        ...
        allCourses.forEach(course => {
            ...
            course.cursarAprob?.forEach(pid => {
                if (nodeIds.has(pid)) {
                    newEdges.push({
        ...
        setNodes(prevNodes => {
            return baseGraph.nodes.map(baseNode => ({
                ...
                data: {
                    ...
                    isDimmed: prevNodes.find(n => n.id === baseNode.id)?.data.isDimmed || false,
                    isHighlighted: prevNodes.find(n => n.id === baseNode.id)?.data.isHighlighted || false,
                    isSelected: prevNodes.find(n => n.id === baseNode.id)?.data.isSelected || false,
                }
            }));
        });

        setEdges(prevEdges => {
            const edgesChanged = prevEdges.length !== baseGraph.edges.length ||
