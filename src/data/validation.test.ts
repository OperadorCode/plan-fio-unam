 ...
 *    (`cursarReg`, `cursarAprob`, `rendirAprob`) existan dentro del conjunto de materias.
    ...
    Object.values(allPlans).forEach((plan: StudyPlan) => {
        describe(`Plan: ${plan.name} (${plan.id})`, () => {
            ...
            it('Debe cumplir con el esquema Zod (Estructura base)', () => {
                ...
                if (!result.success) {
                    console.error(`Error en esquema del plan ${plan.id}:`, result.error.format());
                }
            ...
            it('Todas las correlativas deben existir dentro del plan', () => {
                ...
                Object.values(plan.coursesData).flat().forEach(c => allCourseIds.add(c.id));
                    ...
                    courseVars.forEach(c => {
                        ...
                        refs.forEach(refId => {
                            ...
                            if (!allCourseIds.has(refId)) {
                                throw new Error(`La materia '${c.name}' (${c.id}) referencia a '${refId}' que NO existe en el plan.`);
                            }
                        });
                    });
                };

                expect(() => checkRefs(Object.values(plan.coursesData).flat())).not.toThrow();

                if (plan.electivesData) {
                    expect(() => checkRefs(Object.values(plan.electivesData!).flat())).not.toThrow();
                }
            });

            it('Los slots de electivas deben tener grupos válidos definidos en electivesData', () => {
                const slots = Object.values(plan.coursesData).flat().filter(c => c.isElectiveSlot);
                ...
                slots.forEach(slot => {
                    ...
                    if (plan.electivesData) {
                        ...
                        if (!options || options.length === 0) {
                            throw new Error(`El slot '${slot.name}' apunta al grupo '${group}' que no existe o está vacío en electivesData.`);
                        }
                    } else {
                        throw new Error(`El slot '${slot.name}' requiere electivas, pero el plan no tiene 'electivesData'.`);
                    }
                });
            });
