 ...
 *    - Todas las correlativas de 'requiredRegularToCourse' tienen que estar REGULAR o APROBADA.
 *    - Todas las correlativas de 'requiredApprovedToCourse' tienen que estar APROBADA.
 * 2. Para estar APROBADO (Final):
 *    - Se debe cumplir lo anterior (implícito si ya llegaste a aprobarla, pero chequeamos igual).
 *    - Todas las correlativas de 'requiredApprovedToFinal' (finales previos) tienen que estar APROBADA.
 *
...
export const validateCourseStatus = (
  ...
  while (hasChanges && iterations < 10) {
    ...
    allCoursesRaw.forEach((course) => {
      ...
      const effectiveCourse = resolveEffectiveCourse(
        course,
        selectedElectives,
        plan
      );
...
const checkGlobalRules = (
  ...
  if (course.isElectiveSlot || course.electiveGroup) {
    ...
    return (
      checkYearApproved(1, plan, status) &&
 ...
 */
const checkYearApproved = (
  year: number,
  plan: StudyPlan,
  status: CourseStatusMap
): boolean => {
  // Validación básica del año para evitar accesos indebidos
  if (year < 1 || year > 6) return false;

  const courses = plan.coursesData[year.toString()];
  if (!courses) return true; // Si no hay cursos definidos para ese año, "cumple" (no bloquea) o debería ser false según lógica.
  // Asumimos que si el año es válido pero no hay materias, no hay nada que aprobar.
 ...
 */
const checkSemesterApproved = (
  year: number,
  semesterSignature: string,
  plan: StudyPlan,
  status: CourseStatusMap
): boolean => {
  if (year < 1 || year > 6) return false;
  ...
  return courses
    .filter(
      (c) => c.regimen.includes(semesterSignature) || c.regimen === "Anual"
    )
...
const resolveEffectiveCourse = (
  ...
  if (course.isElectiveSlot) {
    ...
    if (selectedOptionId) {
      const optionData = plan.electivesData?.[course.electiveGroup!]?.find(
