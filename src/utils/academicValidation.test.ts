...
const createCourse = (
  id: string,
  requiredRegularToCourse: string[] = [],
  requiredApprovedToCourse: string[] = [],
  requiredApprovedToFinal: string[] = []
): Course => ({
  id,
  name: `Course ${id}`,
  regimen: "Anual",
...
describe("academicValidation Logic", () => {
  ...
  it("debería permitir aprobar una materia sin correlativas (A)", () => {
    ...
    const result = validateCourseStatus(
      initialStatus,
      mockPlan,
      emptyElectives,
      emptyExamPlan
  ...
  it("debería permitir cursar C solo si A está APROBADA", () => {
    ...
    const result1 = validateCourseStatus(
      status1,
      mockPlan,
      emptyElectives,
      emptyExamPlan
    ...
    const result2 = validateCourseStatus(
      status2,
      mockPlan,
      emptyElectives,
      emptyExamPlan
  ...
  it('Finales: Requiere correlativas ("requiredApprovedToFinal") aprobadas para aprobar final', () => {
    // 'D' requiere 'A' aprobado SOLO para el final (requiredApprovedToFinal).
