...
export const ElectiveSlotRow: React.FC<Props> = ({ slot }) => {
    ...
    const currentPlan = careerPlans[careerId as keyof typeof careerPlans] as unknown as StudyPlan;
    ...
    const allCoursesById = useMemo(() => {
        ...
        return combined.reduce((acc, c) => ({ ...acc, [c.id]: c }), {} as Record<string, Course>);
    }, [currentPlan]);

    const getMissing = (option: Course, forCursar: boolean) => {
        return getMissingPrerequisites(option, forCursar, courseStatus, allCoursesById);
    };
    ...
    const canRegularize = activeOption
        ? getMissing(activeOption, true).regular.length === 0 && getMissing(activeOption, true).approved.length === 0
        : false;

    const canApprove = activeOption && canRegularize
    ...
    const getContainerClasses = () => {
        let classes = "group relative flex flex-col transition-all duration-300 ease-in-out border-l-4 cursor-pointer ";

        if (currentStatus === 'approved') classes += "bg-green-50/40 dark:bg-green-900/10 border-green-500 ";
        else if (currentStatus === 'regular') classes += "bg-blue-50/40 dark:bg-blue-900/10 border-blue-400 ";
        else if (activeOption && !canRegularize) classes += "bg-gray-50 dark:bg-gray-900 border-gray-300 opacity-90 ";
        else classes += "bg-white dark:bg-gray-800 border-indigo-300 hover:shadow-md ";
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 gap-3">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                            <span className={`font-semibold text-sm sm:text-base ${currentStatus ? 'text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-300'}`}>
                                {activeOption ? activeOption.name : slot.name}
                            </span>

                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${activeOption ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-2 min-h-[20px]">
                            {activeOption ? (
                                <>
                                    <span className="font-mono text-[10px] opacity-70">{activeOption.id}</span>
                                    <span>•</span>
                                    <span>{activeOption.hours !== '-' ? `${activeOption.hours}hs` : 'Semestral'}</span>
                                    {!canRegularize && (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {activeOption && (
                            <div className="flex items-center gap-1 sm:gap-2 mr-2" onClick={(e) => e.stopPropagation()}>
                                {/* BOTÓN REGULAR */}
                                <button
                                    onClick={handleRegularChange}
                                    disabled={!canRegularize}
                                    title={!canRegularize ? "Faltan correlativas de cursada" : "Regularizar"}
                                    className={`
                                flex items-center justify-center w-8 h-8 rounded-lg border transition-all duration-300
                                ${(currentStatus === 'regular' || currentStatus === 'approved')
                            ...
                            `}
                                ...
                                <button
                                    ...
                                    disabled={!canApprove}
                                    title={!canApprove ? "Faltan finales anteriores" : "Aprobar Final"}
                                    className={`
                                flex items-center justify-center w-8 h-8 rounded-lg border transition-all duration-300
                            ...
                            `}
                                >
                                    {!canApprove && canRegularize ? <Lock size={12} /> : <Check size={16} strokeWidth={3} />}
                                </button>
                            </div>
                        )}
                    ...
                    >
                        <div className="mt-2 ml-1 sm:ml-4 border-l-2 border-indigo-200 pl-2 sm:pl-4 space-y-2 pb-2">
                            ...
                            {options.map(opt => {
                                ...
                                const isAvailable = missing.regular.length === 0 && missing.approved.length === 0;
                                const isSelected = opt.id === selectedOptionId;
                                const isSelectedInOtherSlot = Object.keys(selectedElectives).some(key => {
                                    if (key === slot.id) return false;
                                    const otherSlot = Object.values(currentPlan.coursesData).flat().find(c => c.id === key);
                                    if (otherSlot?.electiveGroup === slot.electiveGroup && selectedElectives[key] === opt.id) {
                                        return true;
                                    }
                                    return false;
                                });
                                ...
                                return (
                                    <button
                                        key={opt.id}
                                        onClick={(e) => { e.stopPropagation(); if (!isDisabled && isAvailable) handleSelect(opt.id); }}
                                    ...
                                    ${!isAvailable && !isSelected && !isDisabled ? 'opacity-70 bg-gray-50 dark:bg-gray-900 grayscale-[0.3]' : ''}
                                `}
                                    >
                                        <div className="flex justify-between items-start w-full">
                                            <div className={`font-semibold ${isSelected ? 'text-indigo-900' : 'text-gray-700 dark:text-gray-200'}`}>
                                                {opt.name}
                                            </div>
                                        ...
                                        {!isAvailable && !isSelected && !isDisabled && (
                                            <div className="text-xs text-red-400 mt-1 pl-2 border-l-2 border-red-200">
                                                ...
                                                {[...missing.regular, ...missing.approved].slice(0, 3).join(", ")}
