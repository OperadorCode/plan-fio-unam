...
export const CourseNode = memo(({ data }: NodeProps<CourseNodeData>) => {
  const { course, status, isDimmed, isHighlighted, isSelected, criticality } =
    data;
  ...
  let borderColor = isElectiveSelection
  ...
  if (
    ...
    if (year === 1) bgColor = "bg-slate-50 dark:bg-slate-900/20";
    else if (year === 2) bgColor = "bg-gray-50 dark:bg-gray-900/20";
    else if (year === 3) bgColor = "bg-zinc-50 dark:bg-zinc-900/20";
    else if (year === 4) bgColor = "bg-neutral-50 dark:bg-neutral-900/20";
    else if (year >= 5) bgColor = "bg-stone-50 dark:bg-stone-900/20";
  }
  ...
  const opacityClass = isDimmed
    ? "opacity-10 grayscale transition-all duration-300"
  ...
  if (isHighlighted || isSelected) {
    shadowClass =
      "shadow-2xl ring-4 ring-blue-400 ring-offset-4 dark:ring-offset-gray-900 scale-110 z-50";
    bgColor = "bg-white dark:bg-gray-700";
  }

  const isCritical = criticality >= 3 && status !== "approved";
  const periodText = useMemo(
    () => getPeriodText(course.regimen),
    [course.regimen]
  );
  ...
  return (
    <div className={`relative group ${opacityClass}`}>
      <div
        className={`w-72 p-5 rounded-2xl border-2 ${borderColor} ${bgColor} ${shadowClass} transition-all cursor-pointer relative overflow-hidden`}
      >
        {isCritical && (
          <div
            className="absolute top-0 right-0 p-1.5 bg-orange-100 dark:bg-orange-900/30 rounded-bl-xl border-l border-b border-orange-200 dark:border-orange-700/50"
            title="Materia Crítica"
          >
            <Flame
              size={16}
              className="text-orange-500 animate-pulse-slow"
              fill="currentColor"
            />
          </div>
        )}
        ...
        <Handle
          ...
          isConnectable={false}
          className="!bg-transparent !border-0 !w-full !h-full !top-0 !left-0 !rounded-none"
        />
        <Handle
          type="source"
        ...
        <div className="flex justify-between items-start mb-3 pr-6">
          <span
            className={`text-xs font-mono font-bold tracking-widest px-2 py-1 rounded ${
              isElectiveSelection
