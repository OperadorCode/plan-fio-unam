...
const ExamPlanner: React.FC = () => {
  ...
  const [openSelectorTurnId, setOpenSelectorTurnId] = useState<string | null>(
    null
  );
  ...
  const allCoursesById = useMemo(() => {
    const plan = allPlans[activePlanId] || careerPlans[careerId];
    if (!plan) return {};
  ...
  }, [careerId, activePlanId]);

  const availableSubjects = useMemo(() => {
    return Object.values(allCoursesById)
      .filter((course) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (scrollContainerRef.current) {
        const firstFutureCard = scrollContainerRef.current.querySelector(
          '[data-future="true"]'
        ) as HTMLElement;
        if (firstFutureCard) {
          const scrollLeft =
            firstFutureCard.offsetLeft - scrollContainerRef.current.offsetLeft;
    ...
    <div className="space-y-4 animate-fade-in w-full">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-bold text-gray-800 dark:text-white leading-none">
              Mesas de Examen
            </h2>
      ...
      >
        {(() => {
          const pastTurns = examDates.filter((t) => getDaysDiff(t.end) < 0);
          const futureTurns = examDates.filter((t) => getDaysDiff(t.end) >= 0);
          const lastPastTurn =
            pastTurns.length > 0 ? [pastTurns[pastTurns.length - 1]] : [];

          const visibleTurns = [...lastPastTurn, ...futureTurns];
          ...
          return visibleTurns.map((turn) => {
            ...
            return (
              <div
                ...
                className={`
                        /* RESPONSIVE WIDTHS */
                        w-[85vw] sm:w-[320px] md:w-[280px] lg:w-full
              ...
              >
                <div
                  className={`p-3 border-b border-gray-100 dark:border-gray-700 rounded-t-xl ${bgHeader}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    >
                      {isFuture && (
                        <>
                          <Clock size={10} /> {daysToStart} días
                        </>
                      )}
                      {isCurrent && (
                        <>
                          <CalendarCheck size={10} /> En Curso
                        </>
                      )}
                      {isPast && <>Finalizado</>}
                    </span>
                  ...
                  <h3
                    className="font-bold text-gray-800 dark:text-white text-sm leading-tight mb-0.5 truncate"
                    title={turn.name}
                  >
                    {turn.name}
                  </h3>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">
                    {formatDateRange(turn.start, turn.end)}
                  </p>
                </div>
                  ...
                  {selectedCourseIds.length > 0 ? (
                    <div className="space-y-1.5 max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
                      {selectedCourseIds.map((id) => {
                        ...
                        const hasRegularStatus = courseStatus[id] === "regular";
                        return (
                          <div
                            key={id}
                            className="group flex justify-between items-center bg-gray-50 dark:bg-gray-700/50 p-1.5 rounded border border-gray-100 dark:border-gray-700"
                          >
                            <div className="flex items-center gap-2 overflow-hidden">
                              ...
                              <div className="overflow-hidden">
                                <p
                                  className="font-medium text-xs text-gray-700 dark:text-gray-200 truncate"
                                  title={course.name}
                                >
                                  {course.name}
                                </p>
                                <p className="text-[9px] text-gray-400 leading-none">
                                  {hasRegularStatus ? "Regular" : "Libre"}
                                </p>
                              </div>
                            </div>
                            {!isPast && (
                    ...
                    {isSelectorOpen && (
                      <div className="absolute bottom-full left-0 right-0 mb-2 mx-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-600 z-20 animate-fade-in flex flex-col max-h-[220px]">
                        <div className="p-2 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-600 rounded-t-xl flex justify-between items-center">
                          ...
                          <button onClick={() => toggleSelector(turn.id)}>
                        ...
                        <div className="overflow-y-auto p-1 custom-scrollbar">
                          {availableSubjects.map((course) => {
                            ...
                            const hasRegularStatus =
                              courseStatus[course.id] === "regular";
                            return (
                              <button
                                key={course.id}
                                onClick={() =>
                                  handleToggleSubject(turn.id, course.id)
                                }
                                className={`
