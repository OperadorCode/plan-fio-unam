...
export const CalendarLegend = memo(({ visibleTypes, onToggleType }: CalendarLegendProps) => {
    return (
            ...
            {Object.keys(LEGEND_COLORS).map(key => {
                ...
                return (
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-2 py-1 rounded-full transition-all duration-300 ${isVisible ? 'bg-gray-100 dark:bg-gray-700 opacity-100' : 'opacity-50 grayscale'}`}
                    >
                        <span className={`w-2 h-2 rounded-full ${LEGEND_COLORS[k]}`}></span>
                        <span className="text-[10px] text-gray-600 dark:text-gray-300 font-medium">{LEGEND_LABELS[k]}</span>
                    </button>
                )
            })}
        </div>
