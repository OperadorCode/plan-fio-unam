...
export const CalendarScheduleView = memo(({ events, onSelectDate }: CalendarScheduleViewProps) => {
                        ...
                        <div className="space-y-3 px-4 py-3">
                            {monthEvents.map((ev, idx) => {
                                ...
                                return (
                                    <div
                                        key={`${ev.date}-${idx}`}
                                        onClick={() => onSelectDate(new Date(ev.date + 'T12:00:00'))}
                                    ...
                                    >
                                        <div className="flex flex-col items-center min-w-[3rem]">
                                            <span className="text-xs text-gray-500 dark:text-gray-400 capitalize">{dayName.slice(0, 3)}</span>
                                            <span className="text-xl font-bold text-gray-800 dark:text-gray-200">{dayNum}</span>
                                        </div>

                                        <div className="flex-1">
                                            <p className="text-gray-800 dark:text-gray-100 font-medium text-sm">{ev.description}</p>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className={`text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 flex items-center gap-1`}>
                                                    <Tag size={10} />
