...
const CourseIdSchema = z
  .string()
  .min(1, "El ID de la materia no puede estar vacío");
const HoursSchema = z.union([z.string().min(1), z.number().positive()]);
...
export const CourseSchema = z
  .object({
    ...
    requiredRegularToCourse: z.array(z.string()),
    requiredApprovedToCourse: z.array(z.string()),
    requiredApprovedToFinal: z.array(z.string()),
  ...
  .refine(
    ...
    {
      message:
        "Si la materia es un slot electivo, debe tener definido un 'electiveGroup'",
      path: ["electiveGroup"],
    }
  );
